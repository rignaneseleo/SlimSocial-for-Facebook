...
4. **Video download — reassess, the stated blocker may not exist.** This plan excludes video on the grounds that MSE delivers `blob:` sources that cannot be handed to a downloader. A DOM recon of the live mobile layout on 2026-08-06 found that video posts carry a **`data-video-url` attribute holding a plain `https://video-*.xx.fbcdn.net/…` URL** — six of them in one feed, no blob involved. If that holds generally, video download needs no request interception at all: read the attribute, hand it to the same `downloadMedia` helper this plan already builds, and the overlay work is nearly all reusable. Verify on more than one feed and on reels specifically before committing to it, since the recon saw ordinary video posts only.
