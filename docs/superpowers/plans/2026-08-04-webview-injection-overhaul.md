...
**The one risky task is 7.** Everything else is invisible to Facebook: it changes what the app injects, not what the app asks for. Task 7 changes what Facebook serves, so it can regress the whole feed rather than one feature — which is why it re-runs Task 1's recon (Step 8), owns any selector change that recon forces, and is gated first in Task 10 Step 1. Its two constants are not guesses: both are known-good production values, verified against the live site to serve the mobile layout in the regions where a desktop agent breaks. `consts_test.dart` pins the feed agent verbatim so a well-meaning version bump cannot silently switch the served layout back.
