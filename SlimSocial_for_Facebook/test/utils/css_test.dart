...
''',
  ...
  group('comment reaction count vs replies control (#368)', () {
    final code = CustomCss.commentReactionRepliesCss.code;

    test('ships switched on', () {
      expect(
        CustomCss.commentReactionRepliesCss.key,
        'comment_reaction_replies',
      );
      expect(CustomCss.commentReactionRepliesCss.isEnabled(), isTrue);
    });

    test('is not offered as a user-facing toggle', () {
      final keys = CustomCss.cssList.map((c) => c.key);

      expect(keys, isNot(contains('comment_reaction_replies')));
    });

    test('keys off role and aria-label, not hashed classes', () {
      // The x* classes in the captured comet tree are regenerated by
      // Facebook's build; a rule pinned to them stops matching silently.
      expect(code, contains('div[role="article"]'));
      expect(code, contains('div[role="button"][aria-label*="reactions"]'));
      expect(RegExp(r'\.x[0-9a-z]{4,}').hasMatch(code), isFalse);
    });

    test('does not rely on the legacy badge class', () {
      // span._59tg is not in the comet tree; the dark theme may keep it.
      expect(code, isNot(contains('_59tg')));
    });

    test('moves the replies row with space, not stacking', () {
      // z-index only picks which one is drawn on top; the two still share the
      // same pixels and taps.
      expect(code, contains('padding-top: 18px !important'));
      expect(code, isNot(contains('z-index')));
    });
